...
function createHtmlFileList(files, dir, useIcons, view) {
  ...
  html += files.map(function (file) {
	...
	//MODIFICARE:
	//QUi un if:
	//se è un immagine/gif costruisci due righe html per la visualizzazione
	//se è un video/webm usa il player video per far partire il webm in loop.
	//creare anche stylesheet se necessario.
	//console.log(tipofile);
	
	
	/*
	
	if(tipofile == "icon-image"){
	
	
	var prejoin = '<li><a href="'
	...
	*/
	...
	else if(tipofile == "icon-directory"){
	var prejoin = '<li>'
	  + '<a href="'
      + escapeHtml(normalizeSlashes(normalize(path.join('/'))))
      + '" class="' + escapeHtml(classes.join(' ')) + '"'
...
function iconStyle(files, useIcons) {
  ...
  for (i = 0; i < files.length; i++) {
    ...
    //se è directory, imposta la variabile per non caricare le immagini
    if(isDir){
    	tipofile = "icon-directory";
    }
